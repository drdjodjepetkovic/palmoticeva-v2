...
import type { ConversationalAgentInput } from '@/types/ai-types';
...
export default function AiTestPage() {
  ...
  const getMenstrualDataForAgent = async (userId: string): Promise<MenstrualData> => {
    try {
      ...
      if (!lastCycle) {
        return { error: "Nema regularnih ciklusa.", isPeriod: false, isFertile: false, isOvulation: false };
      }
  ...
  const handleTest = async () => {
    ...
    try {
      ...
      const response = await runConversationalAgentV2({
        question: question,
        history: [],
        userId: user?.uid,
