  ...
  useEffect(() => {
    ...
    if (newBadges.length > 0) {
      ...
      newBadges.forEach(badgeKey => {
        ...
        if (badge) {
          toast({
            title: T_el('gamification_dialog_title') as string,
  ...
  return (
    <>
      ...
      <Dialog open={isAiAssistantOpen} onOpenChange={setIsAiAssistantOpen}>
        <DialogContent className="sm:max-w-[425px] md:max-w-2xl h-[80vh] flex flex-col p-0 gap-0 bg-background">
          <DialogHeader className="px-6 py-4 border-b flex flex-row items-center justify-between">
            <div>
              <DialogTitle>{T_el('ai_assistant_title', 'Palmotićeva AI Assistant')}</DialogTitle>
              <DialogDescription>
                {T_el('ai_assistant_desc', 'Ask me anything about your health or appointments.')}
              </DialogDescription>
            </div>
          </DialogHeader>
