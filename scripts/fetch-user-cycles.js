...
async function fetchCycles() {
    try {
        ...
        if (!dailyEventsSnapshot.empty) {
            dailyEventsSnapshot.forEach(doc => {
                console.log(`Event: ${doc.id}`, doc.data());
            });
        } else {
            console.log('No dailyEvents found.');
        }

        // Check for 'cycles' collection under user
        ...
        if (!cyclesSnapshot.empty) {
            ...
            console.log('No cycles collection found.');
        }
