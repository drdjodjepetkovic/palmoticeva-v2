...
import { DailyEvent } from '@/core/types';
...
export function MonthlyCalendarView({
    ...
    const getDayClassNames = (day: Date) => {
        ...
        return cn(
            'h-12 w-12 flex flex-col items-center justify-center rounded-full text-sm relative transition-all hover:bg-accent',
            !isSameMonth(day, currentMonth) && 'text-muted-foreground/50',
            isToday(day) && !isSelected && 'bg-primary/20 text-primary-foreground font-bold ring-2 ring-primary/20',
            isSelected && 'bg-primary text-primary-foreground font-bold shadow-md scale-105',
            {
                'bg-destructive/80 text-destructive-foreground hover:bg-destructive/90': !isSelected && periodDays.has(dateKey),
                'bg-destructive/30 hover:bg-destructive/40': !isSelected && predictedPeriodDays.has(dateKey),
                'bg-primary/40 hover:bg-primary/50': !isSelected && fertileDays.has(dateKey),
                'border-2 border-primary': !isSelected && ovulationDays.has(dateKey),
            }
        );
    };
    ...
    return (
        <div className="space-y-4">
            <div className="flex items-center justify-between bg-card p-2 rounded-lg shadow-sm">
            ...
            <div className="grid grid-cols-7 gap-1 text-center text-xs text-muted-foreground font-medium py-2">
                {weekDays.map(day => <div key={day}>{day}</div>)}
            </div>
            <div className="grid grid-cols-7 gap-1">
                {calendarGridDays.map((day, index) => (
                    <div key={index} className="flex justify-center items-center aspect-square">
