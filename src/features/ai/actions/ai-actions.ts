...
import type { ConversationalAgentInput, ConversationalAgentOutput } from '@/features/ai/types';
import { getAgentContextData } from '@/features/ai/data/agent-data';
import { analyzeCycleHealth } from '@/features/ai/lib/cycle-health';
...
export async function runConversationalAgentV2(input: ConversationalAgentInput): Promise<ConversationalAgentOutput> {
    ...
    try {
        ...
        // Get context data
        // Map 'se' to 'sr' if needed, or just cast. 'se' was legacy code for Serbian?
        // Actually AppLanguage is 'sr' | 'en' | 'ru'.
        const langCode = (input.language === 'se' ? 'sr' : input.language) as any;
        const {
        ...
        } = await getAgentContextData(langCode);
        ...
        if (input.menstrualData && !input.menstrualData.error) {
            ...
            // We cast to any to bypass strict Cycle type checks for this analysis helper
            const cycles: any[] = (input.menstrualData.pastCycles || []).map((pc, index) => ({
                id: `temp-cycle-${index}`,
            ...
            if (parsedResponse.action && parsedResponse.action.type === 'LOG_PERIOD' && input.userId) {
                ...
                try {
                    ...
                    if (actionDate > maxAllowedDate) {
                        ...
                        const { logPeriod } = await import('@/lib/actions/cycle-actions');
                        await logPeriod(input.userId, actionDate);
