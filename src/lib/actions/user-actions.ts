...
import { type Cycle, type Notification, type UserProfile } from '@/core/types';
...
export async function getUserFiles(userId: string): Promise<{ files?: UserFile[]; error?: string }> {
    try {
        // Check if storageAdmin is initialized (it might not be in client-side only envs, but this is a server action)
        if (!storageAdmin) {
...
export async function generateCalendarLinks(input: CalendarExportInput): Promise<CalendarExportOutput> {
    try {
        ...
        const lang = (rawLang as string) === 'se' || rawLang === 'sr' ? 'sr' : rawLang;
