import { Cycle, DailyEvent } from '@/types/user';
...
export function analyzeCycleHealth(cycles: Cycle[], events: DailyEvent[]): CycleHealthReport {
    ...
    // Only consider closed cycles (have an end date) or calculate from start-to-start
    // A more robust way for "cycle length" is usually start date to next start date.
    // But here our Cycle type has startDate and endDate (which usually means bleeding end).
    // Wait, Cycle type definition: startDate, endDate. Usually endDate in these apps means the end of the *period* bleeding, not the cycle.
    // Let's assume cycle length is diff between startDates.
    ...
    for (let i = 0; i < sortedCycles.length - 1; i++) {
        ...
        // Filter out crazy outliers (e.g. < 15 days or > 100 days) if needed, but let's keep it simple for now
    ...
    // This depends on endDate being populated.
    ...
    // If variance in cycle lengths is low (< 3-4 days), it's regular.
